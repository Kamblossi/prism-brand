...
def _update_site_config(values: dict):
    """Write non-secret PrismERP branding keys into the active site's site_config.json.

    This function must only write branding keys. It must not touch database credentials,
    encryption keys, Redis settings, or other runtime secrets.
    """
    site_config_path = Path(frappe.get_site_path("site_config.json"))
...
def apply_branding():
    """Apply PrismERP surface branding to the active site.

    This function is safe to run repeatedly.
    It is intended for after_install and after_migrate hooks.
    """
    # Persist branding keys in site_config.json (survives restarts).
    _update_site_config(BRAND_SITE_CONFIG)
